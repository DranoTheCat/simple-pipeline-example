...
JENKINS_IAM_PROFILE='arn:aws:iam::146311103463:instance-profile/Example_EKS_Nodegroup_Role' # Profile for above
...
SG_ID=$(aws ec2 describe-security-groups --group-names inbound-jenkins-access |jq -r ".SecurityGroups[0].GroupId" | tr '\n' ' ')
...
#aws ec2 run-instances --image-id ${AMI_ID} --count 1 --instance-type ${INSTANCE_TYPE} --key-name ${KEYPAIR_NAME} --security-group-ids ${SG_ID} --subnet-id ${SUBNET_ID} --iam-instance-profile Arn=${JENKINS_IAM_PROFILE} --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=jenkins-host}]' | tee /dev/null # The | tee gets rid of the annoying the implied "less" AWS cli does
...
ssh -i ${PRIVATE_IDENTITY} -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ubuntu@${EC2_IP} sudo cat /var/lib/jenkins/secrets/initialAdminPassword
